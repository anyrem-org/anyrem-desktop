...
function position(window: BrowserWindow) {
  ...
  window.setPosition(Math.round(area.x + (area.width - width) / 2), area.y + 72);
}

function createQuickWindow(route: QuickRoute, preload: string, isDev: boolean) {
  let shownAt = 0;
  const window = new BrowserWindow({
    title: route === 'quick-search' ? 'Remember Anything — Quick Search' : 'Remember Anything — Quick Create',
    width: 720,
  ...
  else window.loadFile(path.join(path.dirname(preload), '../../dist/index.html'), {hash: `/${route}`});
  window.setVisibleOnAllWorkspaces(true, {visibleOnFullScreen: true});
  // Windows can emit blur while foreground permission is settling after a global shortcut.
  window.on('blur', () => { if (Date.now() - shownAt > 500) window.hide(); });
  window.on('show', () => { shownAt = Date.now(); });
  return window;
}
