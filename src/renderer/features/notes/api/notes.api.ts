...
export const getNotes = (filters: NoteFilters) =>
  apiClient
    .get<Paginated<ApiNote>>('/notes', { params: filters })
    .then(({ data }) => ({ ...data, items: data.items.map(mapNote) }));
export const getNote = (id: string) =>
  apiClient.get<ApiNote>(`/notes/${id}`).then(({ data }) => mapNote(data));
export const createNote = (input: NoteInput) =>
  apiClient.post<ApiNote>('/notes', input).then(({ data }) => mapNote(data));
export const updateNote = ({ id, input }: { id: string; input: NoteInput }) =>
