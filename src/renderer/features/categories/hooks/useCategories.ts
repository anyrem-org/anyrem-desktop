...
export function useCreateCategory() { const client = useQueryClient(); return useMutation({ mutationFn: createCategory, onSuccess: () => client.invalidateQueries({ queryKey: categoryKeys.all }) }); }
export function useUpdateCategory() { const client = useQueryClient(); return useMutation({ mutationFn: updateCategory, onSuccess: (category) => { client.invalidateQueries({ queryKey: categoryKeys.all }); client.invalidateQueries({ queryKey: categoryKeys.detail(category.id) }); } }); }
export function useDeleteCategory() { const client = useQueryClient(); return useMutation({ mutationFn: deleteCategory, onSuccess: (_data, id) => { client.removeQueries({ queryKey: categoryKeys.detail(id) }); client.invalidateQueries({ queryKey: categoryKeys.all }); } }); }
